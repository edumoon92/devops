...
Handle Alert with invalid action
    Click Link    Click Me!
    Run Keyword And Expect Error    ValueError: Invalid alert action 'INVALID'.    Handle Alert    INVALID
    Alert Should Be Present
...
Handle Alert with custom timeout
    ...
    Click Button    Slow alert
    Run Keyword And Expect Error    Alert not found in 1 millisecond.    Handle Alert    ACCEPT    1 ms
    Handle Alert    timeout=3.14 seconds

Alert Should Not Be Present
    Alert Should Not Be Present
    Click Link    Click Me!
    Run Keyword And Expect Error    Alert with message 'ALERT!' present.    Alert Should Not Be Present

Alert Should Not Be Present with custom actions
    [Setup]    Go To Page "javascript/dynamic_content.html"
    Click Button    Change the title
    Run Keyword And Expect Error    Alert with message 'Really change the title?' present.    Alert Should Not Be Present    action=LEAVE
    Run Keyword And Expect Error    Alert with message 'Really change the title?' present.    Alert Should Not Be Present    action=DISmiss
    Wait For Title Change    Original
...
Alert Should Not Be Present with custom timeout
    ...
    Run Keyword And Expect Error    Alert with message 'Alert after 500ms!' present.    Alert Should Not Be Present    timeout=0.99999

Alert Should Be Present
    Run Keyword And Expect Error    Alert not found in 1 second.    Alert Should Be Present
    Click Link    Click Me!
    Alert Should Be Present
...
Alert Should Be Present with message validation
    ...
    Click Link    Click Me!
    Run Keyword And Expect Error    Alert message should have been 'foo bar' but it was 'ALERT!'.    Alert Should Be Present    foo bar
...
Alert Should Be Present with custom timeout
    Click Button    Slow alert
    Run Keyword And Expect Error    Alert not found in 1 millisecond.    Alert Should Be Present    timeout=1ms
