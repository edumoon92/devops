...
Get Web Element Should Fail If Element Is Not Found
    Run Keyword and Expect Error    Element with locator 'id=non_existing_elem' not found.    Get WebElement    id=non_existing_elem
...
Get Element Attribute Value Should Be Should Be Succesfull error and error messages
    Run Keyword And Expect Error    Test Fail Custom Message    Element Attribute Value Should Be    id=image_id    href    http://non_existing.com    message=Test Fail Custom Message
    Run Keyword And Expect Error    Element 'id=image_id' attribute should have value 'http://non_existing.com' (str) but its value was 'None' (nonetype).    Element Attribute Value Should Be    id=image_id    href    http://non_existing.com
    Run Keyword And Expect Error    Element with locator 'id=non_existing' not found.    Element Attribute Value Should Be    id=non_existing    href    http://non_existing.com
    Run Keyword And Expect Error    Element 'link=Target opens in new window' attribute should have value 'http://localhost:7000/html/indéx.html' (str) but its value was 'http://localhost:7000/html/index.html' (str).    Element Attribute Value Should Be    link=Target opens in new window    href    http://localhost:7000/html/indéx.html

Get Horizontal Position
    ${pos}=    Get Horizontal Position    link=Link
    Should Be True    ${pos} > 0
    Run Keyword And Expect Error    Element with locator 'non-existent' not found.    Get Horizontal Position    non-existent
...
Get Element Size
    ...
    Run Keyword And Expect Error    Element with locator 'non-existent' not found.    Get Element Size    non-existent
...
Cover Elements should throw exception when locator is invalid
    Run Keyword And Expect Error    No element with locator '//img?@src="inexistent"?' found.    Cover Element    //img[@src="inexistent"]
