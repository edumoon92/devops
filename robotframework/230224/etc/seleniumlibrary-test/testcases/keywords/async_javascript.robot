...
Suite Teardown    Run Keywords    Close All Browsers    Set Selenium Timeout    5 seconds
Test Setup        Go To Page "javascript/dynamic_content.html"
Resource          ../resource.robot

*** Test Cases ***
Should Not Timeout If Callback Invoked Immediately
    ${result} =    Execute Async Javascript    JAVASCRIPT    arguments[arguments.length - 1](123);
    Should Be Equal    ${result}    ${123}

Execute Async Javascript With ARGUMENTS and JAVASCRIPT Marker
    Execute Async Javascript    ARGUMENTS    123    JAVASCRIPT    alert(arguments[0]);
...
Should Timeout If Script Does Not Invoke Callback With A Zero Timeout
    Run Keyword And Expect Error    TimeoutException:*    Execute Async Javascript    window.setTimeout(function() {}, 0);
...
Should Timeout If Script Does Not Invoke Callback With Long Timeout
    ...
    Run Keyword And Expect Error    TimeoutException:*    Execute Async Javascript    var callback = arguments[arguments.length - 1]; window.setTimeout(callback, 1500);

Should Detect Page Loads While Waiting On An Async Script And Return An Error
    Set Selenium Timeout    0.5 seconds
    ${status}    ${error}    Run Keyword And Ignore Error    Execute Async Javascript    window.location = 'javascript/dynamic';
    Should Match Regexp    ${error}    (WebDriverException\:|JavascriptException\:)

Should Catch Errors When Executing Initial Script
    ${status}    ${error}    Run Keyword And Ignore Error    Execute Async Javascript    throw Error('you should catch this!');
    Should Match Regexp    ${error}    (WebDriverException\:|JavascriptException\:)
